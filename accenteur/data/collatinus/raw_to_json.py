...
# into JSON Objects in the global data file 'data.json'.
...
# Add the "long" quantity to the vowels which are long by position (of course if they haven't any length indicated):
# Rule:
# If a vowel is followed by two consonantics, it is long by position.
# Exception:
# If the 1st consonantic is a 'begadkefat' (hebrew mnemonic), i.e. in "bgdcpt",
# and the 2d one is a 'liquid', i.e. 'l' or 'r', the vowel is common (and thus treated as breve in ecclesiastic context).
...
this_dir = os.path.abspath("/home/frromain/Documents/Langues/Latin/latin-ecclesiastic-accents/accenteur/data/collatinus/")

# Delete the contents of data.js:
this_file = open(this_dir + "/data.json", "w", encoding="utf-8")
this_file.write("");
this_file.close()
...
# Models in collatinus/modeles.la are of this form:
...
json_path = open(this_dir + "/data.json", "a", encoding="utf-8")
